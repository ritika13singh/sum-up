...
def submitText():
    ...
    print(username)
    sourceText = request.form.get('content-target1')
    summaryText = Model.returnSummary(sourceText)
    returnObj = [sourceText,summaryText]
    print(returnObj)
    status = storage.add_summary(username, summaryText)
    print(status)
    if(status):
        flash("Summary generated and stored successfully!")
        return render_template('summarizer.html',
                              returnArr = returnObj)
    else:
